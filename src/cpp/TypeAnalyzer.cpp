...
bool TypeAnalyzer::isASubtypeOfB(PureType* a, PureType* b) {
...
bool TypeAnalyzer::isAExactlyB(PureType* a, PureType* b) {
	if(a->type == TYPE_MATCHALL || b->type == TYPE_MATCHALL) return true;
...
void TypeAnalyzer::assertFQNeedIsNotCircular(string classname, string otherclassname) {
	...
	vector<SpecializableVarDecl*>* recurse = reference->findFullyQualified(otherclassname)->getNeeds();
...
void TypeAnalyzer::assertFQClassCanProvide(string provider, SpecializablePureType* binding) {
	...
	vector<SpecializableVarDecl*>* recurse = reference->findFullyQualified(fqname)->getNeeds();
	for(vector<SpecializableVarDecl*>::iterator it = recurse->begin(); it != recurse->end(); ++it)
	try {
...
boost::optional<PureType*> TypeAnalyzer::getCommonSubtypeOf(PureType* a, PureType* b) {
	if(a->type == TYPE_UNUSABLE || b->type == TYPE_UNUSABLE) return boost::optional<PureType*>(); // this maybe should return TYPE_UNUSABLE...but unusable types aren't necessarily similar at all
	if(a->type == TYPE_MATCHALL) return boost::optional<PureType*>(new PureType(*b));
	if(b->type == TYPE_MATCHALL) return boost::optional<PureType*>(new PureType(*a));

	if(a->type == TYPE_OPTIONAL || b->type == TYPE_OPTIONAL) {
		PureType* optional = a->type == TYPE_OPTIONAL ? a : b;
		PureType* other = a == optional ? b : a;
		...
		boost::optional<PureType*> nonoptcommon = getCommonSubtypeOf(optional->typedata.optional.contained, other);

		if(!nonoptcommon) {
			return nonoptcommon;
		}

		PureType* common = new PureType(TYPE_OPTIONAL);

		// [Text?, Text] becomes Text?
		common->typedata.optional.contained = *nonoptcommon;
		return boost::optional<PureType*>(common);
	}

	if(a->type != b->type) return boost::optional<PureType*>();

	if(a->type == TYPE_LIST) {
		// [Text[], Num[]] and [Printer[], DisabledPrinter[]] are both common to nothing
		if(!isAExactlyB(a, b)) return boost::optional<PureType*>();
		// but [Text[], Text[]] is common to Text[]
		return boost::optional<PureType*>(new PureType(*a));
		...
		if(isASubtypeOfB(a, b)) return boost::optional<PureType*>(new PureType(*b));

		if(isASubtypeOfB(b, a)) return boost::optional<PureType*>(new PureType(*a));
		...
		if(!common || common->second == "") {
			...
			PureType* classType = new PureType(TYPE_CLASS);
			classType->typedata._class.classname = strdup(common->second.c_str());
			return boost::optional<PureType*>(classType);
		}
	} else if(a->type == TYPE_PARAMETERIZED || a->type == TYPE_PARAMETERIZED_ARG) {
		if(a->typedata.parameterized.label == string(b->typedata.parameterized.label)) return boost::optional<PureType*>(new PureType(*a));
		// TODO: lower/upper bounds comparison
		return boost::optional<PureType*>();
	}
	...
	if(a->type == TYPE_LAMBDA) {
		...
		// however, void --fn() is not a subtype of Bool -- fn() as you probably guessed
		if(a->typedata.lambda.returntype == NULL || b->typedata.lambda.returntype == NULL) {
			PureType* newlambda = new PureType(*a);
			...
			return boost::optional<PureType*>(newlambda);
		}


		boost::optional<PureType*> commonReturn = getCommonSubtypeOf(a->typedata.lambda.returntype, b->typedata.lambda.returntype);
		if(!commonReturn) {
			PureType* newlambda = new PureType(*a);
			delete newlambda->typedata.lambda.returntype;
			newlambda->typedata.lambda.returntype = NULL;
...
bool TypeAnalyzer::isAutoboxedType(PureType* type, PureType** boxed) {
	...
	if(type->type == TYPE_LIST) {
		*boxed = new PureType(TYPE_CLASS);
		PureType* loweredtype = new PureType(*type->typedata.list.contained);
		(*boxed)->typedata._class.classname = strdup("List");
		(*boxed)->typedata._class.parameters = new PureTypeArray();
		addPureTypeToPureTypeArray(loweredtype, (*boxed)->typedata._class.parameters);
		return true;
	}
...
bool TypeAnalyzer::isException(PureType* type) {
	PureType exception(TYPE_CLASS);
	exception.typedata._class.classname = strdup("Exception");
	bool ret = isASubtypeOfB(type, &exception);
	return ret;
}

int TypeAnalyzer::getArrayReferenceLevel(PureType& type) {
...
bool TypeAnalyzer::hasArgParameterization(PureType* type) {
...
bool TypeAnalyzer::hasArgParameterization(PureTypeArray* typearray) {
	if(typearray == NULL) {
		return false;
	}
